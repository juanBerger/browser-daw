...
function create_fragment$6(ctx) {
	...
	return {
		c() {
			...
			attr(div1, "class", "line svelte-1d17bea");
			attr(div2, "class", "clip svelte-1d17bea");
		},
		m(target, anchor) {
...
function instance$5($$self, $$props, $$invalidate) {
	...
	const unsub = framesPerPixel.subscribe(fpp => {
		const zoom = () => {
			...
			$$invalidate(7, start = updatePosition(delta, start));
 ...
 */
	...
	onMount(async () => {
		if (fileId !== null) {
			...
			$$invalidate(6, _maxSvgWidth = framesToPixels([lineData.sampleLength / lineData.channels], get_store_value(framesPerPixel)));
